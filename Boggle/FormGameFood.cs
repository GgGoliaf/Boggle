...
namespace Boggle
{
    public partial class FormGameFood : Form
    {
        Color[] wordColors = { Color.LightGreen, Color.LightBlue, Color.Orange, Color.Pink, Color.Cyan};

        string[] dictionary = { "æхсыр", "фæткъуы", "кæрдо", "джитъри", "балджын",};
        ...
        void FillField()
        {
            ...
            {
                dataGridViewFieldFood.Rows.Add();
                for (int column = 0; column < 5; column++) // 5 столбцов
                {
                    memoryColors[dataGridViewFieldFood[column, row]] = Color.White; // - начальный цвет для ячейки
                    if (count < letters.Length)
                    {
                        dataGridViewFieldFood[column, row].Value = letters[count];  
                        count++;
                    }
                }
            }
        ...
        private void FormGameFood_Load(object sender, EventArgs e)
        {
            //dataGridViewFieldFood.Rows.Add(5);
            StartGame();
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (timeLeft > 0)
            {
                ...
                labelTimeInFood.Text = $"{minutes}:{seconds:D2}"; //D2 добавляет "0" перед цифрой, если она одна
            }
            else
            {
        ...
        private void dataGridViewFieldFood_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewCell clickedCell = dataGridViewFieldFood[e.ColumnIndex, e.RowIndex]; // нажатая ячейка

            if (selectedCells.Count == 0)
            {
                SelectCell(clickedCell);
                dataGridViewFieldFood.CurrentCell = null; //снять состояние активности с текущей ячейки
            ...
            dataGridViewFieldFood.CurrentCell = null; // убирает системное выделение
        }

        private void buttonCheckWordInFood_Click(object sender, EventArgs e)
        {
            string currentWord = "";
